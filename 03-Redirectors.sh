...
# After performing mysql -t -uroot -pExpenseApp@1 <sample >out you can still see it printing some output on the screen
...
# Now if you want the output in one separate file and error in other file then we can execute the below command
   #     mysql -t -uroot -pExpenseApp@1 <sample >out 2>error
...
# if you want combine output and error use the below command
  # mysql -t -uroot -pExpenseApp@1 <sample &>out  now both output & error will be in the out file nothing on screen

# The single > symbol will overwrite  the previous content
# The double >> symbol will Append to previous content

# Suppose the out file is already having the content and if give mysql -t -uroot -pExpenseApp@1 <sample >>out now in the
